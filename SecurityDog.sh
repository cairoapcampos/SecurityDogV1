...
echo
echo "############################################################################"
echo "###               Definindo as configurações de Port Knocking            ###"
echo "############################################################################"
PortKnock
echo
echo "############################################################################"
...
ctvulpkg=$(debsecan --suite stretch --only-fixed --format packages | wc -l)
...
then
    ...
    echo
    apt install $(debsecan --suite stretch --only-fixed --format packages)
else
    echo
